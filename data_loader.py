...
def get_train_test_loader(data_dir,
    ...
    data_path = "/local/home/aruzzi/personal_calibration_files_sted"
    ...
    test_set = GazeDataset(dataset_path=data_path_test, keys_to_use=datastore["test_specific"],
...
class GazeDataset(Dataset):
    def __init__(self, dataset_path: str, keys_to_use: List[str] = None, transform=None, is_shuffle=True,
        ...
        for num_i in range(0, len(self.selected_keys)):
            if is_train:
                file_path = os.path.join(self.path, self.selected_keys[num_i][:-3] + "_nsample_2_iter_0.h5")
    ...
    def __getitem__(self, idx):
        ...
        # if self.hdf is None:
        if self.is_train:
            self.hdf = h5py.File(os.path.join(self.path, self.selected_keys[0][:-3] + "_nsample_2_iter_0.h5"), 'r', swmr=True)
